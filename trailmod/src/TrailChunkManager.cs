    ...
    public class TrailChunkManager
    {
        ...
        */
        ...
        public void AddOrUpdateBlockPosTrailData( IWorldAccessor world, Block block, BlockPos blockPos, Entity touchEnt )
        {
            lock (trailModificationLock)
            {
                ...
                if (!touchIsPlayer)
                {
                    ...
                    {
                        if (player?.Entity is not EntityPlayer ep)
                            continue;

                        // Mounted directly (elk/horse/etc.)
                        if (ep.WatchedAttributes != null &&
                            ep.WatchedAttributes.HasAttribute("mountedOn") &&
                            BlockCenterHorizontalInEntityBoundingBox(touchEnt, ep.Pos.AsBlockPos))
                        {
                            touchIsPlayer = true;
                            break;
                        }

                        // Vehicle / sled / cart proximity
                        double dx = ep.Pos.X - touchEnt.Pos.X;
                        double dz = ep.Pos.Z - touchEnt.Pos.Z;
                ...
                if (trailChunkEntries.ContainsKey(chunk))
                {
                    if (trailChunkEntries[chunk].ContainsKey(blockTrailID))
                    {
                        ...
                        bool shouldTryTransform = entryToUpdate.BlockTouched(touchEnt.EntityId, touchEnt.Pos, touchEnt.World.ElapsedMilliseconds);
                    ...
                    {
                        TrailBlockPosEntry trailBlockEntry = new TrailBlockPosEntry(blockPos, touchEnt.EntityId, touchEnt.Pos, touchEnt.World.ElapsedMilliseconds, 1);
            ...
            lock (trailModificationLock)
            {
                if (trailBlockTouchTransforms.ContainsKey(blockID))
                {
                    ...
                    if (!touchIsPlayer)
                    {
                        foreach (IPlayer player in world.AllOnlinePlayers)
                        {
                            if (player?.Entity is EntityPlayer ep &&
                                ep.WatchedAttributes != null &&
                                ep.WatchedAttributes.HasAttribute("mountedOn") &&
                                BlockCenterHorizontalInEntityBoundingBox(touchEnt, ep.Pos.AsBlockPos))
                            {
                                touchIsPlayer = true;
                    ...
                    if (touchEnt is EntityPlayer touchPlayer)
                    {
        ...
        private static ETrailTrampleType CanTramplePlant( Block plantBlock, BlockPos plantPos, Block groundBlock, BlockPos groundPos, Entity touchEnt )
        {
            ...
            if (plantBlock.BlockMaterial == EnumBlockMaterial.Plant)
            {
                ...
                if (TMGlobalConstants.fernTrampling)
                {
                    ...
                    {
                    if (plantBlock.BlockMaterial == EnumBlockMaterial.Plant &&
                       plantBlock.Code.Path.Contains("fern"))
                            return ETrailTrampleType.DEFAULT;
        ...
        private static bool ShouldDropPlantOnTrample(Block block)
        {
            ...
            {
                if (block?.BlockMaterial == EnumBlockMaterial.Plant &&
                    block.Code?.Path.Contains("fern") == true)
                    return false;

                if (block?.BlockMaterial == EnumBlockMaterial.Plant &&
                    block.Code?.Path.Contains("tallgrass") == true)
                    return false;

                if (block?.BlockMaterial == EnumBlockMaterial.Plant &&
                    block.Code?.Path.Contains("lupine") == true)
                    return true;

                string code = block.Code.FirstCodePart();

                if (block.Code?.Path?.Contains("flower") == true)
                    return true;

                if (block.Code?.Path?.Contains("tallfern") == true)
                    return false;
            }
        ...
        private static void ResolveTrampleType(IWorldAccessor world, ETrailTrampleType trampleType, BlockPos upPos, Block upBlock, Block groundBlock )
        {
            ...
            switch(trampleType)
            {
                ...
                case ETrailTrampleType.TALLGRASS:
                    ...
                    bool groundIsTrail = groundBlock?.Code?.Path?.Contains("trail") == true;
                    ...
                    else
                    {
                        ...
                        string groundCode = groundBlock?.Code?.Path ?? "";

                        if (groundCode.Contains("peat") || groundCode.Contains("rawclay"))
                        {
                            if (TMGlobalConstants.foliageTrampleSounds)
                            {
                                world.BlockAccessor.BreakBlock(upPos, null, 0);
